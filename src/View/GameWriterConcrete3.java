...
public class GameWriterConcrete3 extends Application {
	static GameWriterConcrete3 myWriter;
	private static final String FILE_DESTINATION = "src/xml/GameFreePathZombie.xml";
...
//		world.setObstacles(obstaclePoints);
		...
		GameObjectSimple g = new TestTower(2,100,100);
		
		try {
			g = new TestTower(2, 330, 330);
			MoverUser m = new MoverUser();
			m.setGraphic(g.getGraphic());
			g.setMover(m);
			g.setWeapon(new NullWeapon());

			world.addObject(g, new PointSimple(0,300));
        }
		...
		GridCellFromPoint c = new GridCellFromPoint(new CoordinateTransformer(10, 10, ViewConcrete2.getWorldWidth(),
                ViewConcrete2.getWorldHeight()), g);
	...
	public static void main(String[] args) {
		myWriter = new GameWriterConcrete3();
		myWriter.writeGame();
	}
